...
import DropDownDatastoreAction from "@/components/Website-Search/DropDownDatastoreAction"
import DataStoreDeleteDialog from "@/components/Website-Search/DataStoreDeleteDialog";
import DataStoreEditDialog from "@/components/Website-Search/DataStoreEditDialog";
import DataStoreAddDialog from "@/components/Website-Search/DataStoreAddDialog";
import EmbedWebsiteDialog from "@/components/Website-Search/EmbedWebsiteDialog";

import { useAuth } from '@/contexts/authContext';
import { db } from '@/config/firebase';
import { collection, getDocs, limit, orderBy, query, startAfter, updateDoc, where } from 'firebase/firestore';
...
const WebsiteSearchPage = () => {
    ...
    const addDataStore = (docStore: WebDateStore) => {
    ...
    const togglePublic = async (id: string) => {
        try {
            const q = query(collection(db, "website-search"), where("assistantId", "==", id));
            const querySnapshot = await getDocs(q);
                ...
                <table className="w-full text-sm text-left rtl:text-right text-blue-100 dark:text-blue-100">
                    <thead className="text-xs text-white uppercase bg-[#4a6cf7] dark:text-white">
                        <tr>
                            ...
                            <th scope="col" className="px-6 py-3 text-center hidden sm:table-cell">
                                URL
                            </th>
                            <th scope="col" className="px-6 py-3 text-center">
                                Public
                    ...
                    <tbody>
                        {!loading && datastore && datastore?.map((item, index) => (
                            <tr className={`${index === 0 ? "" : "border-t"} text-black dark:text-white dark:bg-[#282c32] dark:border-[#222222]`}>
                                <th scope="row" className="px-6 font-medium whitespace-nowrap dark:text-blue-100 text-center text-black dark:text-white">
                                    {(pageCnt - 1) * pageSize + index + 1}
                                </th>
                                <td className="px-6 py-4 text-center text-black dark:text-white" title={item?.description}>
                                    {item?.name}
                                </td>
                                <td className="px-6 text-center text-black dark:text-white hidden sm:table-cell" title={item?.description}>
                                    {item?.url}
                                </td>
                                <td className="px-6 text-center text-black dark:text-white" title={item?.description}>
                                ...
                                <td className="px-6 text-center text-black dark:text-white" title={item?.description}>
                                    <div className='relative z-50'>
                                        <DropDownDatastoreAction id={item?.id} handleDelete={handleDelete} handleEdit={handleEdit} handleEmbedDialog={handleEmbedDialog} />
                                    </div>
                                </td>
                            </tr>
                        ))}
